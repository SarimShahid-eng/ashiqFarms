    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item active">{{__('Fourth Head')}}</div>
            </div>
        </div>
        <div class="section-body">
            ...
            <div class="col-md-12 text-center">
              {{-- @foreach(range('A','Z') AS $pagination)
                <a href="{{ route('expense.child.forthheads.show',['search'=>$pagination]) }}" class="badge badge-primary">{{ $pagination }}</a>
              @endforeach --}}
            <div class="col-12">
                <div class="card repeater">
                    <div class="card-header">
                        <h4>{{__('Fourth Head')}}</h4>
                        @if(Auth::user()->type == "company" || Auth::user()->can('add fourth subhead') )
                        
                          <div class="card-header-action">
                              <a href="{{ route('expense.child.forthheads.add') }}"  class="btn btn-icon icon-left btn-primary" id="add_head">
                                  <i class="fas fa-plus"></i>{{__('Add Fourth SubHead')}}
                              </a>
                          </div>
                        @endif
                    </div>
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td style="border: 1px solid #000">
                              @if(Auth::user()->type == "company" || Auth::user()->can('edit fourth subhead') )
                                <a href="{{ route('expense.child.subheads.edit',['id'=>$head->id]) }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                              @endif
                              @if(Auth::user()->type == "company" || Auth::user()->can('delete fourth subhead') )
                          ...
                          {{-- @dd($val); --}}
                                <tr>
                                  ...
                                  <td style="border: 1px solid #000">{{ @$val->forth_name }}</td>
                                  <td style="border: 1px solid #000">
                                    @if(Auth::user()->type == "company" || Auth::user()->can('edit fourth subhead') )
                                      <a href="{{ route('expense.child.forheads.edit',['id'=>$val->id]) }}" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                                    @endif
                                    @if(Auth::user()->type == "company" || Auth::user()->can('delete third subhead') )
                                      <a href="javscript:void(0)" class="btn btn-danger" onclick="return confirm('Are you sure?') ? run_ajax('{{ route('expense.child.forheads.delete',['id'=>$val->id]) }}',''): '' "><i class="fas fa-trash"></i></a>
                                    @endif
                                    </td>
                                </tr>
                                 @endforeach
