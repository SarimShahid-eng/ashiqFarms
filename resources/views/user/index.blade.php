                    ...
                    <div class="card-header">
                        <div class="d-flex justify-content-between w-100">
                            <h4>{{__('Manage User')}}</h4>
                                <a href="{{ route('roles.create') }}"  data-title="{{__('Create New User')}}" class="btn btn-icon icon-left btn-primary">
                        ...
                        <div class="staff-wrap">
                            <div class="row">
                                @foreach($users as $user)
                                    <div  style="cursor:pointer;" class=" col-lg-auto col-md-4 col-sm-6">
                                        <div class="staff staff-grid-view pb-0">
                                            <div class="more-action">
                                                @if($user->is_active==1)
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu" style="width:auto;min-width:auto;">

                                                                <a href="{{ route('roles.edit',['id'=>$user->id]) }}" class="dropdown-item font-weight-bold" data-title="{{__('Update User')}}" style="color:#000">{{__('Edit')}}</a>
                                                                {{-- <a href="{{ route('users.destroy',['id'=>$user->id]) }}" class="dropdown-item"   data-title="{{__('Delete User')}}">{{__('Delete')}}</a> --}}
                                                                <a href="javascript:void(0)" onclick="return confirm('Are you sure?') ? run_ajax('{{ route('users.destroy',['id'=>$user->id]) }}',''): '' " class="dropdown-item font-weight-bold" style="color:#000">Delete</a>
                                            ...
                                            <div class="contact-img">
                                                {{-- <img src="{{(!empty($user->avatar)) ? check_file(asset(Storage::url("avatar/".$user->avatar))) : check_file(asset(Storage::url("avatar/avatar.png")))}}" class="rounded-circle"> --}}
                                                @if(!empty($user->avatar))
                                                    @if(file_exists(public_path('uploads/profile_images/'.$user->avatar)))
                                                        <img src="{{ asset('/uploads/profile_images/'.$user->avatar) }}" alt="">
                                                    @else
                                                        <img src="{{ check_file(asset(Storage::url("avatar/avatar.png"))) }}" alt="">    
                                                    @endif
                                                @else
                                                <img src="{{ check_file(asset(Storage::url("avatar/avatar.png"))) }}" alt="">    
                                                @endif
                                            ...
                                            <div class="main-info mb-4">
                                                <h2 data-session="{{@$user->id}}" class="user_login m-0 font-style font-weight-bold">{{$user->name}}</h2>
                                                {{-- <p class="font-style"> {{$user->type}}</p> --}}
                                            </div>
                                            <div class="meta-info mb-3">
                                                <p style="color:#000" class="font-weight-bold"> {{$user->email}}</p>
                                            ...
                                            @if(\Auth::user()->type == 'super admin')
                                                ...
                                                <div class="btn-wrap">
                                                    ...
                                                    <button class="btn" title="Total Vendors">
                                                        <i class="fas fa-users"></i>
